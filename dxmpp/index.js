...
var Dxmpp = /** @class */ (function () {
    ...
    Dxmpp.prototype.get_files = function (files_st) {
        var files_a_st = files_st.getChildren('item');
        var files = [];
        files_a_st.forEach(function (file) {
            files.push({
    ...
    ;
    Dxmpp.prototype.send = function (user, message, group, files) {
        var _this = this;
        this.$.ready(function () {
            ...
            if (stanza.is('message')) {
                ...
                if (stanza.attrs.type == 'chat') {
                    ...
                    if (body) {
                        ...
                        var files = [];
                        var files_st = stanza.getChild('files');
                        if (files_st) {
                            files = _this.get_files(files_st);
                        }
                        _this.events.emit('chat', user, message, files);
                    }
                    ...
                    if (body) {
                        ...
                        _this.events.emit('groupchat', Dxmpp.get_room_data(stanza), message, sender, stamp, files);
                    }
                }
            }
                                    ...
                                    if (messages) {
                                        ...
                                        if (item) {
                                            item.forEach(function (element) {
                                                ...
                                                var files = element.getChild("files");
                                                if (files) {
                                                    var list_files_1 = [];
                                                    var item_files = files.getChildren("item");
                                                    item_files.forEach(function (file) {
                                                        var attrs = file.attrs;
                                                        attrs.hash = file.getText();
                                                        list_files_1.push(attrs);
                                                    });
                                                    obj_message.files = list_files_1;
                                                }
                                                list_messages_1.push(obj_message);
                                            });
                                        }
                                    }
