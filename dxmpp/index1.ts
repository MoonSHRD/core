...
class Dxmpp {
    ...
    acceptSubscription(to) {
        this.$.ready(()=>{
            let stanza = new Stanza('presence', {from:this.client.options.jid,to: to, type: 'subscribed'});
    ...
    subscribe(to) {
        this.$.ready(()=>{
            let stanza = new Stanza('presence', {from:this.client.options.jid,to: to, type: 'subscribe'});
            this.client.send(stanza);
        });
    };
    ...
    send(to, message, group) {
        this.$.ready(()=>{
            let stanza = new Stanza('message', {from:this.client.options.jid,to: to, type: (group ? 'groupchat' : 'chat')});
    ...
    join(to, password) {
        this.$.ready(()=>{
            let stanza = new Stanza('presence', {from:this.client.options.jid,to: to}).c('x', {xmlns: NS_ROOMSTATES});
    ...
    register_channel(name, domain, contractaddress, password) {
        this.$.ready(()=>{
            let stanza = new Stanza('presence', {from:this.client.options.jid,to: Dxmpp.hexEncode(name)+"@"+domain, contractaddress:contractaddress, channel:'1'}).
    ...
    invite(to, room, reason) {
        this.$.ready(()=>{
            let stanza = new Stanza('message', {from:this.client.options.jid,to: room}).c('x', {xmlns: NS_ROOMSTATES + '#user'}).c('invite', {to: to});
            if (reason)
    ...
    send_suggesstion(to, text) {
        this.$.ready(()=>{
            let stanza = new Stanza('iq', {id:'suggest', from:this.client.options.jid, to: to, type:'set'})
            ...
            if (stanza.is('message')) {
                ...
                if (from) {
                    if (stanza.attrs.type == 'subscribe') {
                        ...
                        //     }
                        // }
                        this.events.emit('buddy', id, state, statusText, resource);
